...
function DiscountInput({ courseId, currency }: { courseId: number; currency: string }) {
  ...
  const validate = useCallback(async () => {
    ...
    setResult(null);
    try {
      // validate-discount requires enrollment_id — here we just check code validity
      // using a lightweight check without enrollment context (preview mode)
      const res = await apiClient.post('/payments/validate-discount/', {
        code: code.trim().toUpperCase(),
        currency,
        // enrollment_id omitted — backend returns 400; we catch that gracefully
      });
      setResult(res.data);
    } catch (err: unknown) {
      const msg =
      ...
      if (msg === 'enrollment_id and code are required.') {
        // Code format looks fine but we can't calculate without enrollment — show generic success hint
        ...
        {!isLoading && !error && courses.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {courses.map((course) => {
              ...
              const priceIsRegional =
                price ? REGIONAL_CURRENCIES.has(price.currency) : false;
          ...
          <div className="space-y-6 text-sm text-gray-200">
            {[
              ...
              },
              {
                ...
                a: 'We are integrating Stripe (cards, Google/Apple Pay) and Paystack (cards, bank transfer, mobile money) — details will appear at checkout.',
              },
              {
